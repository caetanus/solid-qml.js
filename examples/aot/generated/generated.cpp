...
QQuickItem *buildCounter(QQmlContext *ctx, CounterState *state)
{
    ...
    sq::begin(v1, ctx);
    QObject::connect(v1, &SolidWidgets::Button::clicked, v1, [=] { state->setCount(sq::add(state->count(), QVariant(1))); });
    { auto __upd = [=] { v1->setText(sq::str(sq::add(sq::add(sq::add(QVariant(QString()), state->label()), QVariant(QStringLiteral(": "))), state->count()))); };
